  ...
  return (
    <div className="mb-6 rounded-2xl overflow-hidden border border-gray-100 shadow-sm bg-white">
      <div className={`flex flex-col md:flex-row ${fa ? 'md:flex-row-reverse' : ''}`}>
        ...
        {/* Info panel */}
        <div className={`md:w-1/2 flex flex-col justify-center p-6 md:p-8 gap-5 ${fa ? 'text-right' : 'text-left'}`}>
          ...
          <div>
            ...
            <h2 className="text-navy-800 text-xl md:text-2xl font-bold leading-snug">
              {fa
                ? 'لوازم خانگی و برقی برای کسب‌وکار و خانه'
                : 'Home & Commercial Appliances for Every Need'}
            </h2>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className={`flex items-start gap-3 ${fa ? 'flex-row-reverse' : ''}`}>
              ...
              <div className={fa ? 'text-right' : 'text-left'}>
                <p className="font-semibold text-navy-700 text-sm">
                  {fa ? 'جستجو و فیلتر پیشرفته' : 'Advanced Search & Filter'}
                </p>
                <p className="text-gray-400 text-xs mt-0.5">
                  {fa ? 'بر اساس برند، دسته‌بندی، قیمت و موجودی فیلتر کنید' : 'Filter by brand, category, price and availability'}
                </p>
              </div>
            </div>
            ...
            <div className={`flex items-start gap-3 ${fa ? 'flex-row-reverse' : ''}`}>
              ...
              <div className={fa ? 'text-right' : 'text-left'}>
                <p className="font-semibold text-navy-700 text-sm">
                  {fa ? 'پشتیبانی ۲۴ ساعته' : '24/7 Support'}
                </p>
          ...
          <a
            ...
            className={`inline-flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white text-sm font-medium px-5 py-2.5 rounded-xl transition-colors ${fa ? 'self-end' : 'self-start'}`}
          >
